...
const createDraftFromTrainingLog = (data: TrainingLogDetail) =>
  data.exercises.map((exercise) => ({
    id: exercise.id,
    exerciseName: exercise.trainingName,
    rest: String(exercise.exerciseSets.rest ?? ""),
    memo: "",
    sets:
...
const TrainingLogEdit = () => {
  ...
  useEffect(() => {
    if (!trainingLogDetail) return;

    setTrainingDate(getDateInputValue(trainingLogDetail.createdTime));
    setBodyWeight(String(trainingLogDetail.bodyWeight ?? ""));
    setMemo(trainingLogDetail.memo ?? "");
    setExercises(createDraftFromTrainingLog(trainingLogDetail));
  }, [trainingLogDetail]);
  ...
  const exerciseOptions = useMemo(() => {
    ...
    return Array.from(new Set([...names, ...fallbackExerciseOptions]));
  }, [exercises]);
            ...
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  モーダルでセットごとの重量、回数、メモを編集します。
                </p>
              </div>
              <Button
                className="w-full sm:w-auto"
                onClick={openNewExerciseDialog}
              >
                種目を追加
              </Button>
            </CardHeader>
